...
namespace MPSTI.PlenoSoft.Core.Azure.CosmosDb.Repository.Interfaces
{
	...
	public interface ICosmosRepository<TCosmosEntity> where TCosmosEntity : ICosmosEntity
	{
		...
		Task<TCosmosEntity> GetByPKAsIdAsync(string partitionKeyValue);
		...
		Task<TransactionalBatchResponse> ExecuteBatchAsync(string partitionKeyValue, BatchAction batchAction);
		Task<Dictionary<string, TransactionalBatchResponse>> ExecuteBatchAsync<TEntity>(IEnumerable<TEntity> lista, BatchAction<TEntity> batchAction) where TEntity : ICosmosEntity;
	}
}